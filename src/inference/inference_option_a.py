...
ENTER_WINDOW   = 2.5   # seconds — short window detects fatigue quickly
EXIT_WINDOW    = 4.0   # seconds — longer window prevents flip-flopping on exit
ENTER_THRESHOLD = 0.55  # tired_ratio in ENTER_WINDOW to go TIRED (was 0.60)
EXIT_THRESHOLD  = 0.35  # tired_ratio in EXIT_WINDOW must be BELOW this to leave TIRED
                         # (was 0.25 — higher here = easier to exit)

YAWN_OPEN_EAR_MIN = 0.80  # norm_ear above this = eyes open during yawn
...
prediction_history = deque()   # holds (timestamp, class_idx) for EXIT_WINDOW seconds
...
while len(calibration_ears) < CALIBRATION_FRAMES and cap.isOpened():
    ...
    cv2.imshow("Project Aegis — Option A (Asymmetric Windows)", frame)
    cv2.waitKey(1)
...
while cap.isOpened():
    ...
    if results.face_landmarks:
        ...
        if len(ear_history) == WINDOW_SIZE:
            ...
            if warmup_elapsed < WARMUP_SECONDS:
                ...
                # keep only up to EXIT_WINDOW seconds
                ...
                enter_ratio = (sum(v for _, v in enter_votes) /
                               max(len(enter_votes), 1))
                exit_ratio  = (sum(v for _, v in exit_votes) /
                               max(len(exit_votes), 1))

                # yawn with eyes open → immediate TIRED
    ...
    cv2.putText(frame, "[OPT-A] Asymmetric Windows",
