"""
Option C (Pi) — Event Accumulator with Decay
...
# Continuous rates (points/second × dt = FPS-independent)
TIRED_RATE      = 30.0   # pts/s when model says TIRED
ATTENTIVE_RATE  = 16.0   # pts/s subtracted when model says ATTENTIVE
PERCLOS_RATE    =  8.0   # pts/s when perclos > PERCLOS_SPIKE_THRESHOLD

# One-shot bonuses
YAWN_OPEN_BONUS      = 20.0   # yawn with eyes open
LONG_BLINK_BONUS     = 10.0   # completed blink > LONG_BLINK_THRESHOLD seconds

PERCLOS_SPIKE_THRESHOLD = 0.50
LONG_BLINK_THRESHOLD    = 0.40
        ...
        if len(ear_history) == WINDOW_SIZE:
            ...
            if warmup_elapsed < WARMUP_SECONDS:
                ...
                print(f"EAR:{avg_ear_val:.2f} PERCLOS:{perclos:.2f} "
                      f"Pts:{fatigue_points:.1f} State:{alert_state} "
