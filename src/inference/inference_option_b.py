"""
Option B — EMA Fatigue Score  (RECOMMENDED)
Replaces the voting window with a continuous fatigue_score (0.0–1.0).
Uses asymmetric rates: fast rise (detects quickly) and slow decay (controlled recovery).
Yawn with eyes open adds a direct one-shot boost to the score.
"""
import sys
import os
...
# Fatigue score rates (additive per frame — tune these first)
# At ~15 FPS: ATTACK=0.04 → rises 0→0.60 in ~1.0s
#             DECAY=0.015  → falls 1.0→0.25 in ~50 frames ≈ 3.3s
ATTACK_RATE  = 0.04   # added per frame when model says TIRED
DECAY_RATE   = 0.015  # subtracted per frame when model says ATTENTIVE
YAWN_BOOST   = 0.20   # one-shot boost when yawn detected with eyes open

ENTER_THRESHOLD = 0.60  # fatigue_score must exceed this to enter TIRED
EXIT_THRESHOLD  = 0.25  # fatigue_score must drop below this to exit TIRED
...
fatigue_score = 0.0   # THE single score that drives state
...
while len(calibration_ears) < CALIBRATION_FRAMES and cap.isOpened():
    ...
    cv2.imshow("Project Aegis — Option B (EMA Score)", frame)
    cv2.waitKey(1)
...
while cap.isOpened():
    ...
    if results.face_landmarks:
        ...
        # --- yawn tracking (detect rising edge) ---
        ...
        if len(ear_history) == WINDOW_SIZE:
            ...
            if warmup_elapsed < WARMUP_SECONDS:
                ...
                # --- EMA FATIGUE SCORE ---
                if raw_class_idx == 1:
                    fatigue_score = min(1.0, fatigue_score + ATTACK_RATE)
                else:
                    fatigue_score = max(0.0, fatigue_score - DECAY_RATE)

                # yawn with eyes open → direct one-shot boost
                if yawn_open_just_detected:
                    fatigue_score = min(1.0, fatigue_score + YAWN_BOOST)
                    print(f"[YAWN+EYES OPEN] Score boosted → {fatigue_score:.2f}")

                if alert_state == 0 and fatigue_score >= ENTER_THRESHOLD:
                    alert_state = 1
                elif alert_state == 1 and fatigue_score < EXIT_THRESHOLD:
                    alert_state = 0

                diag_text = (f"Score: {fatigue_score:.2f}  "
                             f"EAR: {avg_ear_val:.2f}  MAR: {mar:.2f}")
                print(f"EAR:{avg_ear_val:.2f} PERCLOS:{perclos:.2f} "
                      f"Score:{fatigue_score:.2f} State:{alert_state}")
    ...
    cv2.putText(frame, "[OPT-B] EMA Fatigue Score",
    ...
    # Score bar (visual)
    bar_w = int(fatigue_score * 300)
    bar_color = (0, int(255 * (1 - fatigue_score)), int(255 * fatigue_score))
    cv2.rectangle(frame, (10, 125), (310, 145), (50, 50, 50), -1)
    if bar_w > 0:
        cv2.rectangle(frame, (10, 125), (10 + bar_w, 145), bar_color, -1)
    cv2.putText(frame, f"Fatigue: {fatigue_score:.2f}",
